...
(function(){
  ...
  const relations = [
    {keys:['marketing','sales'],title:'Lead-to-Sales Conversion Gap',text:'Marketing and Sales jointly shape the path from demand to revenue. When both areas score low, the issue is unlikely to be solved by generating more leads alone. The team should align lead quality, qualification, handoff, discovery, proposal, and conversion.',trainingKeys:['sales','marketing']},
    {keys:['sales','customer'],title:'Conversion-to-Retention Gap',text:'A weak sales process combined with weak Customer & CRM practices can create friction both before and after the deal. The team may need stronger discovery, follow-up, customer visibility, retention, and repeat-business practices.',trainingKeys:['sales','customer']},
    {keys:['analytics','technology'],title:'Data-to-Execution Gap',text:'When analytics maturity and technology adoption are both weak, data and tools may exist without becoming part of daily decision-making. The priority is to connect dashboards, CRM, automation, AI, and data discipline to real commercial workflows.',trainingKeys:['analytics','technology']},
    {keys:['technology','people'],title:'Adoption & Capability Gap',text:'Tools do not create value by themselves. Low technology adoption together with people gaps can indicate capability, coaching, workflow, or accountability issues rather than a simple software problem.',trainingKeys:['technology','people']},
    {keys:['strategy','marketing'],title:'Strategy-to-Demand Gap',text:'When Commercial Strategy and Lead Generation & Marketing are both weak, the team may be creating activity before target customers, positioning, and demand priorities are sufficiently clear.',trainingKeys:['strategy','marketing']},
    {keys:['strategy','sales'],title:'Strategy-to-Sales Gap',text:'When strategy and sales execution are both weak, the team may struggle to translate positioning into a consistent sales motion, qualification process, proposal approach, and commercial narrative.',trainingKeys:['strategy','sales']},
    {keys:['people','sales'],title:'Capability-to-Conversion Gap',text:'Low People & Collaboration and Sales Process scores together suggest the issue may include both sales methodology and team capability. Training should combine selling skills with coaching and reinforcement.',trainingKeys:['people','sales']},
    {keys:['marketing','analytics'],title:'Demand-to-Measurement Gap',text:'Low Marketing and Performance & Analytics scores together suggest the organization may be generating activity without a strong view of channel quality, conversion, and contribution to revenue.',trainingKeys:['marketing','analytics']}
  ];
  ...
  const form=$('commercial-check-form'), container=$('question-container');
  const placeholder=$('result-placeholder'), panel=$('result-panel');
  const completionText=$('completion-text'), completionBar=$('completion-bar-fill'), completionNote=$('completion-note');
  const calcBtn=$('calculate-button'), submitNote=$('form-submit-note');
  const overallEl=$('overall-score'), maturityEl=$('maturity-label'), summaryEl=$('result-summary');
  const dimensionsEl=$('dimension-results'), diagnosisEl=$('diagnosis-results'), priorityEl=$('priority-results');
  const trainingEl=$('training-recommendations'), trainingIntro=$('training-intro'), copyBtn=$('copy-result'), waBtn=$('whatsapp-result');
  ...
  function renderQuestions(){
    ...
    dimensions.forEach(dim=>{
      ...
      header.innerHTML='<div><h2>'+dim.title+'</h2><div class="meta">'+dim.description+'</div></div><div class="meta">5 questions</div>';
      section.appendChild(header);
      if(dim.callout){const callout=document.createElement('div');callout.className='ai-callout';callout.textContent=dim.callout;section.appendChild(callout);}
      dim.questions.forEach((q,qi)=>{
        const card=document.createElement('div');card.className='question-card';
        const p=document.createElement('p');p.className='question-text';p.textContent=(qi+1)+'. '+q;card.appendChild(p);
        const options=document.createElement('div');options.className='scale-options';
        for(let score=1;score<=5;score++){
          const label=document.createElement('label');
          const input=document.createElement('input');input.required=true;input.type='radio';input.name=dim.key+'-'+qi;input.value=String(score);
          const span=document.createElement('span');span.textContent=String(score);
          label.appendChild(input);label.appendChild(span);options.appendChild(label);
        }
        card.appendChild(options);section.appendChild(card);
      });
  ...
  function updateProgress(){
    const answered=selectedCount(),pct=Math.round(answered/TOTAL*100);
    completionText.textContent=answered+' of '+TOTAL+' questions answered';completionBar.style.width=pct+'%';
    if(answered===TOTAL){completionNote.textContent='All questions answered. Your result is ready to calculate.';submitNote.textContent='';}
    else{completionNote.textContent='Answer all '+TOTAL+' statements to unlock your result.';}
  }
  function markSelected(){form.querySelectorAll('.scale-options label').forEach(label=>label.classList.toggle('selected',label.querySelector('input').checked));}
  ...
  function findRelations(scores){
    const lowKeys=scores.slice().sort((a,b)=>a.score-b.score).slice(0,4).map(s=>s.key);
    let matches=relations.filter(r=>r.keys.every(k=>lowKeys.includes(k)));
    if(!matches.length){
      if(lowKeys.includes('sales'))matches.push({title:'Commercial Conversion Gap',text:'Several lower-scoring dimensions point toward a conversion challenge. Review qualification, discovery, proposal quality, negotiation, and the customer or marketing processes surrounding sales.',trainingKeys:['sales','customer']});
      else if(lowKeys.includes('marketing'))matches.push({title:'Demand Generation Gap',text:'Several lower-scoring dimensions point toward a demand challenge. Review target customer, channel quality, lead qualification, and the connection between marketing activity and commercial outcomes.',trainingKeys:['marketing','strategy']});
      else matches.push({title:'Commercial Enablement Gap',text:'The lower-scoring dimensions suggest that capability, process, data, or technology should be strengthened as an integrated system rather than addressed in isolation.',trainingKeys:[lowKeys[0],lowKeys[1]]});
    }
    return matches.slice(0,3);
  }
  function getTrainingRecommendations(scores,relationsFound){
    const keys=[];
    relationsFound.forEach(r=>r.trainingKeys.forEach(k=>{if(!keys.includes(k))keys.push(k);}));
    scores.slice().sort((a,b)=>a.score-b.score).forEach(s=>{if(keys.length<3&&!keys.includes(s.key))keys.push(s.key);});
    return keys.slice(0,3).map(k=>trainingMap[k]);
  }
  function showMissing(){
    const missing=[];
    dimensions.forEach(dim=>dim.questions.forEach((_,qi)=>{
      const input=form.querySelector('input[name="'+dim.key+'-'+qi+'"]'),checked=form.querySelector('input[name="'+dim.key+'-'+qi+'"]:checked');
      const card=input.closest('.question-card');card.classList.toggle('missing',!checked);
      let msg=card.querySelector('.missing-message');
      if(!checked){missing.push(true);if(!msg){msg=document.createElement('div');msg.className='missing-message';msg.textContent='Please answer this question.';card.appendChild(msg);}}
  ...
  function calculate(){
    const scores=dimensions.map(dim=>{
      const values=dim.questions.map((_,qi)=>Number(form.querySelector('input[name="'+dim.key+'-'+qi+'"]:checked').value));
      return {key:dim.key,title:dim.title,score:values.reduce((a,b)=>a+b,0)/values.length};
    });
    const avg=scores.reduce((a,b)=>a+b.score,0)/scores.length;
    const level=maturity(avg);
    const relationsFound=findRelations(scores);
    const priorities=scores.slice().sort((a,b)=>a.score-b.score).slice(0,3);
    const recommendations=getTrainingRecommendations(scores,relationsFound);

    overallEl.textContent=avg.toFixed(1)+' / 5';maturityEl.textContent=level[0];summaryEl.textContent=level[1];
    dimensionsEl.innerHTML=scores.map(s=>'<div class="dimension-card"><div class="dimension-header"><strong>'+s.title+'</strong><span class="dimension-score">'+s.score.toFixed(1)+'/5</span></div><div class="bar"><span style="width:'+Math.round(s.score/5*100)+'%"></span></div></div>').join('');
    diagnosisEl.innerHTML=relationsFound.map(r=>'<div class="diagnosis-card"><h4>'+r.title+'</h4><p>'+r.text+'</p><p class="diagnosis-links"><strong>Connected dimensions:</strong> '+r.trainingKeys.join(' + ')+'</p></div>').join('');
    priorityEl.innerHTML=priorities.map((s,i)=>'<div class="priority-item"><strong>'+(i+1)+'. '+s.title+'</strong><div class="meta">Current score: '+s.score.toFixed(1)+'/5 · Gap to 5: '+(5-s.score).toFixed(1)+'</div></div>').join('');
    trainingIntro.textContent='These recommendations are based on the pattern across multiple dimensions and the relationships between them, not on a single low score. They are an initial development recommendation and should be validated with MarkedTraining against business context and root cause.';
    trainingEl.innerHTML=recommendations.map((t,i)=>'<div class="training-item"><span class="training-badge">Priority '+(i+1)+'</span><h4>'+t.name+'</h4><div class="training-meta">Suggested format: '+t.format+'</div><p class="training-reason">'+t.reason+'</p></div>').join('');
    const company=(form.querySelector('[name="company"]').value||'our team').trim();
    const msg='Hi MarkedTraining, I completed the Commercial Check for '+company+'. Overall score: '+avg.toFixed(1)+'/5 ('+level[0]+'). Cross-dimensional priorities: '+relationsFound.map(r=>r.title).join(', ')+'. Recommended training: '+recommendations.map(t=>t.name).join('; ')+'. I would like to consult on the appropriate development plan.';
    waBtn.href='https://wa.me/628971021300?text='+encodeURIComponent(msg);
    placeholder.hidden=true;panel.hidden=false;panel.scrollIntoView({behavior:'smooth',block:'start'});
    return{overall:avg,level:level[0],scores,relations:relationsFound,priorities,recommendations};
  }

  renderQuestions();updateProgress();
  form.addEventListener('change',()=>{markSelected();updateProgress();showMissing();});
  form.addEventListener('submit',e=>{
    ...
    try{
      ...
      if(missing.length){
        ...
        const first=form.querySelector('.question-card.missing');if(first)first.scrollIntoView({behavior:'smooth',block:'center'});
        return;
      }
      window.__commercialResult=calculate();
    }catch(error){
      console.error('Commercial Check calculation error:',error);
      submitNote.textContent='We could not calculate the result. Please refresh the page and try again.';
    }
  });
  copyBtn.addEventListener('click',async()=>{
    ...
    r.relations.forEach(x=>{text+='- '+x.title+': '+x.text+'\n';});
    text+='\nRecommended Training:\n';r.recommendations.forEach((t,i)=>{text+=(i+1)+'. '+t.name+' — '+t.reason+'\n';});
    text+='\nDiscuss with MarkedTraining: 08971021300';
    try{await navigator.clipboard.writeText(text);copyBtn.textContent='Copied!';setTimeout(()=>copyBtn.textContent='Copy Result Summary',1800);}catch(e){alert(text);}
  });
})();
</script>
